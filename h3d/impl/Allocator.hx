...
class ScalePolicy extends AllocatorPolicy {
	...
	public function getNewPageSize( pages : Array<MemoryPage>, reqSize : Int ) : Int {
		...
		if( size < reqSize ) size = reqSize;
		if( size > 0x7FFFFFFF ) throw "Allocation size overflow";
		return Std.int(size);
	}
...
abstract class Allocator {
	...
	public function alloc( size : Int, ?out : MemoryBlock, alignment = 1 ) {
		if( size <= 0 || alignment <= 0 ) throw "Invalid allocation size or alignment";
		if( useMemoryPool ) {
			...
			out = tmp;
		if( alignment < mem.alignment ) alignment = mem.alignment;
		if( alignment != 1 ) {
			var d = size % alignment;
			if( d != 0 ) size += alignment - d;
		}
		out.page = null;
		out.size = size;
		var pos = tryAlloc(size, alignment, out);
		if( pos < 0 ) {
			allocPage(size);
			pos = tryAlloc(size, alignment, out);
	...
	abstract function tryAlloc( size : Int, alignment : Int, out : MemoryBlock ) : Int;
...
class BlockAllocator extends Allocator {
	...
	function tryAlloc( size : Int, alignment : Int, out : MemoryBlock ) {
		while( true ) {
			var p = pages[currentPage];
			if( p == null )
				return -1;
			var pos = AllocatorTools.align(cursor, alignment);
			if( pos + size <= p.size ) {
				out.page = p;
				cursor = pos + size;
				return pos;
	...
	public function reset( forceDispose = false ) {
		...
		while( i > 0 ) {
			if( i < usedPages )
				unusedFrames[i] = 0;
			else if( forceDispose || ++unusedFrames[i] > MAX_KEEP_FRAMES )
				freePage(pages[i]);
			i--;
		}
		cursor = 0;
...
class FreeListAllocator extends Allocator {
	...
	function tryAlloc(size:Int, alignment:Int, out:MemoryBlock):Int {
		for( i => p in pages ) {
			var f = lists[i];
			if( f == null ) {
			...
			var pos = f.alloc(size, alignment, true);
			if( pos >= 0 ) {
				out.page = p;
				return pos;
			}
	...
	public function trim( maxSize : Float ) {
		...
		while( total > maxSize ) {
			...
			for( i => p in pages )
				if( lists[i] != null && lists[i].isFullyFree(p.size) && (largest == null || p.size > largest.size) )
					largest = p;
...
class FreeList {
	...
	public function alloc( size : Int, alignment : Int, bestFit : Bool ) : Int {
		var best = -1, bestLen = -1, bestPos = -1, bestWaste = -1;
		for( i in 0...count ) {
			var len = list[i << 1];
			var pos = list[(i << 1) + 1];
			var alignedPos = AllocatorTools.align(pos, alignment);
			var required = alignedPos - pos + size;
			var waste = len - required;
			if( waste >= 0 && (best < 0 || waste < bestWaste) ) {
				best = i;
				bestLen = len;
				bestPos = alignedPos;
				bestWaste = waste;
				if( !bestFit || required == len ) break;
			}
		}
		if( best < 0 ) return -1;
		var k = best << 1;
		var pos = list[k+1];
		var prefix = bestPos - pos;
		var suffix = bestLen - prefix - size;
		if( prefix == 0 && suffix == 0 )
			merge(k);
		else if( prefix == 0 ) {
			list[k] = suffix;
			list[k+1] = bestPos + size;
		} else {
			list[k] = prefix;
			if( suffix > 0 )
				insert(k + 2, suffix, bestPos + size);
		}
		return bestPos;
	}
	...
	inline function insert( k : Int, size : Int, pos : Int ) {
		var max = count << 1;
		if( max == list.length ) {
			list.push(0);
			list.push(0);
