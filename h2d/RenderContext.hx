...
class RenderContext extends h3d.impl.RenderContext {
	...
	**/
	...
	/**
		Enables a runtime first-level mip-map approximation for minified 2D textures.
		Requires compiling with `-D texture_filtering`.
	**/
	public var runtimeMipMapFilter(default, set) : Bool = false;
	/**
	...
	**/
	...
	function set_runtimeMipMapFilter( v : Bool ) {
		#if texture_filtering
		if( baseShader != null )
			baseShader.runtimeMipMapFilter = v ? 1.0 : 0.0;
		#end
		return runtimeMipMapFilter = v;
	}
