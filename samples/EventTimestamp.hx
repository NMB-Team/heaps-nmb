class EventTimestamp extends hxd.App {
	...
	private function onEvent(event : hxd.Event) {
		...
		trace('${event.kind}: timestamp=${event.timestamp}, age=${limen.platform.Platform.getTime() - event.timestamp}, repeat=${event.isRepeat}');
	}

	private function onPadEvent(event : hxd.Pad.PadEvent) {
		checkTimestamp(event.timestamp);
		trace('${event.kind}: timestamp=${event.timestamp}, age=${limen.platform.Platform.getTime() - event.timestamp}');
	}

	private function checkTimestamp(timestamp : Float) {
		if( timestamp > limen.platform.Platform.getTime() + 0.001 )
